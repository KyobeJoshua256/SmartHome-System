...
// ── CSRF Manager ───────────────────────────────────────────────────────────
const CsrfManager = (() => {
  const REFRESH_INTERVAL_MS = (3600 * 0.75) * 1000;
...
window.fetch = async function csrfAwareFetch(input, init = {}) {
  ...
  if (isMutating && !modifiedInit.headers?.['X-CSRFToken']) {
    ...
    const token = getCsrf();
    if (token) {
      modifiedInit.headers = { ...(modifiedInit.headers || {}), 'X-CSRFToken': token };
    }
  }
  ...
  if ((res.status === 400 || res.status === 403) && isSameOrigin && isMutating) {
    ...
    if (looksLikeCsrf) {
      ...
      if (newToken) {
        ...
        retryInit.headers = { ...(retryInit.headers || {}), 'X-CSRFToken': newToken };
...
function switchSection(name) {
  ...
  // FIX: If switching away from messages, close the active conversation 
  // so incoming messages don't get auto-marked as read and unread badges update correctly.
  if (name !== 'messages' && window.MessagingModule && MessagingModule.initialized) {
    MessagingModule.leaveActiveConversation();
  }
...
function _eatNow() {
  const now = new Date();
  const fmt = part => new Intl.DateTimeFormat('en-UG', { timeZone: TZ, [part]: 'numeric' }).format(now);
  return {
    h: parseInt(fmt('hour'), 10), m: parseInt(fmt('minute'), 10), s: parseInt(fmt('second'), 10),
...
function updateClock() {
  const t = _eatNow();
  if (topbarClock) topbarClock.textContent = `${String(t.h).padStart(2,'0')}:${String(t.m).padStart(2,'0')}:${String(t.s).padStart(2,'0')}`;
  if (topbarDate) topbarDate.textContent = `${t.day}, ${String(t.date).padStart(2,'0')} ${t.month} ${t.year}`;
}
updateClock();
...
window.dashboardSetUnread = function (count) {
  ...
  if (navUnreadBadge) { navUnreadBadge.textContent = count; navUnreadBadge.hidden = count === 0; }
};

const alertBell = document.getElementById('alertBell');
if (alertBell) alertBell.addEventListener('click', () => showToast('info', 'Alerts panel coming soon.'));

CsrfManager.init();

if (window.MessagingModule && DATA.chat && DATA.chat.conversations) {
  MessagingModule.preload(DATA.chat.conversations);
  MessagingModule.initSocket(DATA);
}
