window.MessagingModule = (function () {
  ...
  // CSRF readiness is delegated to window.CsrfManager.ready (userdashboard.js).
  // That promise resolves once the boot-time warm fetch completes, so every
  // await _csrf() call below is automatically gated on the fresh token.
  ...
  /**
   * Seed the conversation list before the messages section is opened.
   * Called by dashboard.js on page boot so the list is ready immediately.
   * @param {Array} conversations - Conversation objects from the server.
   */
  ...
  /**
   * Full initialisation — called once when the messages section is first shown.
  ...
  function init(dashData) {
    ...
    // Extract conversations from the correct path
  ...
  // _warmCsrf is no longer needed: CsrfManager in userdashboard.js performs
  // the boot-time fetch and exposes CsrfManager.ready.  _csrf() awaits that
  // directly, so there is one warm fetch on the page instead of two.
  function _warmCsrf() { /* delegated to CsrfManager */ }

  // ── DOM cache ──────────────────────────────────────────────────────────────

  /** Populate module-level DOM references used throughout the module. */
  function _cacheDOM() {
  ...
  /**
   * Render the sidebar conversation list, optionally filtered by a search string.
   * @param {Array}  convs  - Full conversation array.
   * @param {string} filter - Optional substring filter on display_name.
   */
  ...
  /**
   * Open a conversation: update the header, join the socket room, fetch messages,
   * and POST to mark it read on the server.
   * @param {number} convId
   */
  async function _openConv(convId) {
    ...
    try {
      ...
      // Awaited sequentially (after the GET) so the CSRF token warmed by
      // _warmCsrf() is in place — avoids the guaranteed first-request 400.
      try {
  ...
  /**
   * Replace the message area with a freshly rendered list and start the
   * IntersectionObserver for auto read-marking on scroll.
   * @param {Array} messages - Message objects from the API.
   */
  ...
  /**
   * Build the HTML string for a single message row.
  ...
  function _msgHTML(m) {
    ...
    if (m.message_type === 'system') {
      return `<div class="msg-sys">${esc(m.content)}</div>`;
    }

    const time       = _fmtTime(m.created_at);
    const editedMark = m.is_edited
      ? ' <i class="fas fa-pen" style="font-size:.6rem;opacity:.4"></i>' : '';

    const activeConv = _conversations.find(c => c.conversation_id === _active);
    const isSelfChat = activeConv?.is_self === true;

    // ── Read-status indicator (outgoing only) ──────────────────────────────
    let readStatusHtml = '';
    if (mine) {
      if (isSelfChat) {
        readStatusHtml = `<span class="msg-read-status" title="Saved — ${time}">
        ...
        const countBadge = (rs.status === 'partially_read' || rs.status === 'read') && m.read_count > 0
          ? `<small>${m.read_count}</small>` : '';
        readStatusHtml = `<span class="msg-read-status" title="${esc(rs.label || 'Delivered')} — ${time}">
          <i class="fas ${icon}"></i>${countBadge}
        </span>`;
      } else {
        readStatusHtml = `<span class="msg-read-status" title="Sent — ${time}">
    ...
    const avatarHtml    = !mine
      ? `<div class="msg-avatar" style="background:${avatarColor}">${senderInitial}</div>`
      : '';

    const senderLabel = !mine && m.sender_name !== _username
  ...
  /**
   * Append a single message to the bottom of the chat area.
   * @param {Object} m - Message object.
   */
  function _appendMessage(m) {
    ...
    if (_scrollObserver) {
      ...
      if (newRow && !newRow.classList.contains('mine')) {
        _scrollObserver.observe(newRow);
      }
    }
  }
  ...
  /**
   * POST /chat/messages/{id}/read for each supplied message ID in parallel.
  ...
  /**
   * Attach an IntersectionObserver so received messages are marked read
   * as soon as they enter the viewport.
   */
  function _initScrollObserver() {
    ...
    chatMsgs.querySelectorAll('.msg-row:not(.mine)').forEach(row => {
      _scrollObserver.observe(row);
    });
  }
  ...
  function _initSocket() {
    ...
    _socket = io({ transports: ['websocket', 'polling'], reconnection: true, reconnectionDelay: 1000 });
  ...
  function _bindEvents() {
    ...
    if (convSearch) {
      convSearch.addEventListener('input', debounce(() => {
        _renderConvList(_conversations, convSearch.value);
      }, 200));
    }
    ...
    if (closeNewConv) {
      closeNewConv.addEventListener('click', e => { e.preventDefault(); _closeModal(); });
    }

    if (newConvModal) {
      newConvModal.addEventListener('click', e => { if (e.target === newConvModal) _closeModal(); });
    }
    ...
    if (userSearch) {
      userSearch.addEventListener('input', debounce(_searchUsers, 300));
    }
  }
  ...
  async function _startConversation(targetUserId) {
    try {
      ...
      if (!_conversations.find(c => c.conversation_id === conv.conversation_id)) {
  ...
  /**
   * Return the current CSRF token, waiting for CsrfManager.ready first so
   * no POST ever fires before the boot-time warm fetch has completed.
   */
  async function _csrf() {
    // Await the boot-time warm fetch before reading.
    // Read _data first (CsrfManager writes here), then the island.
    // Never read from <meta name="csrf-token"> — that tag holds the stale
    // rendered token and is not updated by CsrfManager.
  ...
  return { preload, init, get initialized() { return _initialized; } };
