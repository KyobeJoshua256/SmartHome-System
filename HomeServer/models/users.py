...
class User(UserMixin, database.Model):
    ...
    def set_password(self, password: str) -> None:
        ...
        try:
            ...
            self.last_password_change = now_utc()
    ...
    def can_request_otp(self) -> Tuple[bool, Optional[str]]:
        ...
        if self.last_otp_request:
            ...
            hours_since_last = (now_utc() - last_request).total_seconds() / 3600
        ...
        if self.otp_request_count >= self.max_otp_requests_per_hour:
            ...
            if self.last_otp_request:
                last_request = ensure_aware(self.last_otp_request)
                reset_in_seconds = 3600 - (now_utc() - last_request).total_seconds()
    ...
    def record_failed_attempt(self):
        ...
        if self.failed_login_attempts >= self.max_failed_attempts:
            self.is_locked = True
            self.locked_until = now_utc() + timedelta(minutes=self.lockout_duration_minutes)
            import logging
